...
## Задание 4: Риски и SLI/SLO

### Уровень 1 — Бизнес

| SLI | Нормальный SLO | Критический порог | Действие |
|---|---|---|---|
| Доля SKU с прогнозом на завтра | ≥ 95% | < 80% | Алерт, ручная проверка DAG |
| Stockout rate (% товаров с нулевым остатком) | < 5% | > 15% | Экстренный заказ, эскалация |
| Overstock rate | < 10% | > 25% | Акционные цены, пересмотр модели |
| Точность (accuracy_proxy ±50%) | ≥ 75% | < 60% | Немедленное переобучение |

### Уровень 2 — Код / ML-пайплайн

| SLI | Нормальный SLO | Критический порог | Действие |
|---|---|---|---|
| sMAPE модели | ≤ 50% | > 80% | Переобучение, feature review |
| Доля батчей, прошедших валидацию | ≥ 98% | < 90% | Алерт data-инженеру |
| Успешность регистрации модели | ≥ 95% | < 85% | Откат на предыдущую версию |
| Успешность quality gate | ≥ 90% | < 70% | Ревизия порогов и признаков |
| Частота ложных CT-триггеров | < 5% | > 20% | Пересмотр CT-условий |

### Уровень 3 — Инфраструктура

| SLI | Нормальный SLO | Критический порог | Действие |
|---|---|---|---|
| p95 latency прогноза | ≤ 300 мс | > 1000 мс | Кеш, оптимизация признаков (см. ADR-0001) |
| Успешность Airflow DAG run | ≥ 99% | < 95% | Алерт, ручное вмешательство |
| Доступность MinIO/S3 | ≥ 99.9% | < 99% | Failover, retry-логика |
| Использование диска | < 70% | > 90% | Очистка старых батчей (lifecycle rule) |
| Успешность CI/CD пайплайна | ≥ 98% | < 90% | Блокировка merge |

**Подключение человека-оператора** при:
- Любом критическом SLO (stockout rate > 15%, latency > 1000 мс, DAG success < 95%)
- 3+ неуспешных CT-цикла подряд
- Terraform drift (расхождение state и реальной инфраструктуры)
